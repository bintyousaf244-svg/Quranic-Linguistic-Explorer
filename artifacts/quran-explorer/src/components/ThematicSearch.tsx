...
const SUGGESTED_THEMES = [
  'mercy', 'creation', 'prayer', 'justice', 'patience',
  'gratitude', 'forgiveness', 'paradise', 'knowledge', 'light',
];
...
export const ThematicSearch: React.FC<ThematicSearchProps> = ({
  ...
  const handleSearch = async (overrideQuery?: string) => {
    ...
    setResult(null);
    try {
      const resp = await fetch(`${BASE}/api/thematic-search?q=${encodeURIComponent(q)}`);
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold" style={{ color: 'var(--grove-purple)' }}>
                Thematic Search
              </h2>
              <p
                className="text-xs opacity-60"
                style={{ color: 'var(--grove-purple)', fontFamily: '"Amiri", serif' }}
              >
                بحث موضوعي في القرآن الكريم
              </p>
            </div>
          </div>
          <button
        ...
        >
          <form onSubmit={handleSubmit} className="relative mb-4">
            <input
              ...
              placeholder="Enter a theme or concept (e.g. mercy, creation, patience)..."
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {SUGGESTED_THEMES.map((t) => (
              <button
                ...
                className="px-3 py-1 rounded-full text-xs font-semibold capitalize transition-all hover:opacity-80"
                style={{
                  backgroundColor:
                    result?.theme === t
                      ? 'var(--grove-teal)'
                      : 'color-mix(in srgb, var(--grove-teal) 10%, transparent)',
                  color: result?.theme === t ? 'white' : 'var(--grove-teal)',
                }}
              >
                {t}
              </button>
        ...
        <div className="flex-1 overflow-y-auto" style={{ backgroundColor: 'var(--grove-paper)' }}>
          {error ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              ...
              <p className="text-sm opacity-60" style={{ color: 'var(--grove-teal)' }}>
                Analyzing theme and searching verses...
              </p>
            </div>
          ) : result ? (
            <div>
                ...
                >
                  ...
                  {result.arabicConcept && (
                    <div className="flex items-center gap-3 mb-4">
                      <div
                        ...
                        style={{
                          backgroundColor: 'color-mix(in srgb, var(--grove-teal) 12%, transparent)',
                        }}
                      >
                        <Sparkles size={14} style={{ color: 'var(--grove-teal)' }} />
                        <span
                          className="text-xl font-bold"
                          style={{ color: 'var(--grove-teal)', fontFamily: '"Amiri", serif' }}
                        >
                          {result.arabicConcept}
                        </span>
                        {result.transliteration && (
                          <span
                            className="text-xs font-semibold italic opacity-70"
                            style={{ color: 'var(--grove-teal)' }}
                          >
                            ({result.transliteration})
                          </span>
                        )}
                  ...
                  {result.summary && (
                    <p
                      ...
                      style={{ color: 'var(--grove-purple)', opacity: 0.85 }}
                    >
                      {result.summary}
                    </p>
                  )}
                        ...
                        {result.roots.map((r) => (
                          ...
                          >
                            <span
                              ...
                              style={{ fontFamily: '"Amiri", serif', color: 'var(--grove-green)' }}
                            >
                              {r.root}
                            </span>
              ...
              >
                <div className="flex items-center gap-2">
                  <div
                    ...
                    style={{ backgroundColor: 'var(--grove-teal)' }}
                  />
                  <span className="text-sm font-bold" style={{ color: 'var(--grove-teal)' }}>
                    {result.count} verses found
                    {result.verses.length < result.count &&
              ...
              {result.verses.map((v) => (
                <div
                  ...
                  className="px-6 py-4 border-b flex gap-4 group transition-all hover:bg-opacity-50"
                  style={{
                ...
                >
                  ...
                  <div className="shrink-0 mt-1">
                    <div
                      className="text-center rounded-xl px-2 py-1 min-w-[48px]"
                      style={{
                        backgroundColor: 'var(--grove-cream)',
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      ...
                      style={{ color: 'var(--grove-purple)', opacity: 0.9 }}
                      dangerouslySetInnerHTML={{
                        __html: highlightKeyword(v.translation, result.theme),
                      }}
                    />
              ...
              <div className="px-6 py-6 text-center">
                <p
                  ...
                  style={{ color: 'var(--grove-purple)' }}
                >
                  Translation: Saheeh International · Click a root chip to explore all occurrences
                </p>
              </div>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-16 gap-6 px-8">
              <div
                className="w-20 h-20 rounded-3xl flex items-center justify-center opacity-10"
                style={{ backgroundColor: 'var(--grove-cream)', color: 'var(--grove-teal)' }}
              >
                <Lightbulb size={40} />
              </div>
              <div className="text-center">
                <p className="text-base font-bold mb-2" style={{ color: 'var(--grove-purple)' }}>
                  Search by Meaning
                </p>
                <p
                  ...
                  style={{ color: 'var(--grove-purple)' }}
                >
                  Type any English concept — mercy, patience, justice, light — and get every
                  related verse with an AI-powered thematic overview and the Arabic roots behind it.
                </p>
              </div>
            </div>
          )}
