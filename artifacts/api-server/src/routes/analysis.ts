...
function getMessages(type: string, payload: { ayahText?: string; surahName?: string; ayahNumber?: number; word?: string }): PromptMessages {
  ...
  switch (type) {
    case 'grammar':
      return {
        system: `أنت متخصص في إعراب القرآن الكريم وفق منهج كتاب "الإعراب الميسر" لمحمد علي طه الدرة (شركة الدار العربية للموسوعات). هذا الكتاب يتميز بأسلوبه الواضح والمُيسَّر: يُعرب كل كلمة إعراباً مختصراً ودقيقاً في سطر أو سطرين، يذكر الرأي الراجح الواحد دون إطالة في الخلاف، ويشمل إعراب الجمل وتعلّق الجار والمجرور. أسلوبه مباشر، ميسور الفهم، مناسب للطالب والمعلم.`,
        user: `أعرِب الآية الكريمة التالية من سورة ${surahName}، الآية رقم ${ayahNumber}، وفق الأسلوب الميسّر لكتاب "الإعراب الميسر":

﴿${ayahText}﴾

**التعليمات الدقيقة لنمط الإعراب:**
- ابدأ بسرد كلمات الآية تباعاً، كل كلمة في فقرة مستقلة.
- لكل كلمة: اكتب الكلمة بالخط العريض **كلمة:**، ثم اكتب إعرابها في سطر واحد أو اثنين بصيغة واضحة مختصرة.
- الصيغة النموذجية للإعراب: [نوع الكلمة]، [وظيفتها الإعرابية]، [علامة إعرابها ولماذا].
- للأسماء: اذكر (اسم + نوعه إن لزم) + (مبتدأ/خبر/فاعل/مفعول/...) + (مرفوع/منصوب/مجرور وعلامته).
- للأفعال: اذكر (فعل ماضٍ/مضارع/أمر) + (الفاعل ضمير مستتر أو ظاهر) + (الجملة في محل...).
- للحروف: اذكر (حرف + وظيفته) + (مبني على...) + (لا محل له من الإعراب) أو (محله).
- للضمائر المتصلة: اذكرها ضمن إعراب الكلمة التي اتصلت بها.
- للجار والمجرور: اذكر تعلّقهما.
- بعد إعراب جميع المفردات، أضف فقرة قصيرة بعنوان **إعراب الجمل:** تُبيّن فيها محل كل جملة من الإعراب أو عدم محلها.

اكتب الإعراب بالعربية الفصحى، مختصراً واضحاً كما في "الإعراب الميسر"، بلا إطناب ولا ذكر للخلاف إلا إذا كان مشهوراً جداً وفي سطر واحد.`
      };
